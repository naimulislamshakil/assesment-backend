...
export const getSingleProductById = catchAsyncError(
	async (req: Request, res: Response, next: NextFunction) => {
		const { id } = req.params;
		if (!id) {
			return next(new ErrorHandler('Product ID is required', 400));
...
export const getProductsAddedOverTime = async (
	req: Request,
	res: Response,
	next: NextFunction
) => {
...
export const getStockValueByProduct = async (
	req: Request,
	res: Response,
	next: NextFunction
) => {
